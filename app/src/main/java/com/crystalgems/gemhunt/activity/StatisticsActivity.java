...
public class StatisticsActivity extends Activity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        resetButton.setEnabled(Database.getGameDAO().findAllGames().size() != 0
                || Database.getGamePlayerLinkDAO().findAllGamePlayerLink().size() != 0
                || Database.getPlayerDAO().findAllPlayer().size() != 0);
    ...
    public void onClick(View v) {
        ...
        if (v == resetButton) {
            ...
            Database.getPlayerDAO().deleteAllPlayers();
            resetButton.setEnabled(false);
            for (int i = 0; i < statsCardViews.length; i++) {
                statsCardViews[i].setVisibility(View.GONE);
            }
        }
    }
